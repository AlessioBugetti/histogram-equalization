...
void SequentialHistogramEqualization(const unsigned char *input,
  ...
  for (unsigned int &value : cdf) {
    value = static_cast<unsigned int>(
        round(static_cast<double>(value - cdfMin) / (pixelCount - cdfMin) *
              (NUM_BINS - 1)));
  }
...
enum class ScanType { KoggeStone, KoggeStoneDoubleBuffer, BrentKung };

void CudaHistogramEqualization(const unsigned char *hostInput,
  ...
  CalculateHistogram<<<gridDimImage, blockDimImage>>>(
      deviceInput, deviceHistogram, pixelCount);
  ...
  EqualizeHistogram<<<gridDimImage, blockDimImage>>>(deviceOutput, deviceInput,
                                                     deviceCdf, pixelCount);
...
int main() {
  ...
  for (const auto &entry :
    ...
    std::cout << "Average Sequential Time: " << meanSequentialTime << " ms"
              << std::endl;
    std::cout << "Average CUDA Time (Kogge-Stone): " << meanCudaTimeKoggeStone
              << " ms" << std::endl;
    ...
    std::cout << "Speedup (Kogge-Stone Double Buffer): "
              << speedupKoggeStoneDoubleBuffer << std::endl;
    std::cout << "Average CUDA Time (Brent-Kung): " << meanCudaTimeBrentKung
              << " ms" << std::endl;
